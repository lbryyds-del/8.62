...
class Hmdb51(BaseDataset):
    ...
    def _construct_loader(self):
        ...
        self.split_df = self.split_df[
                    self.split_df['video_path'].apply(os.path.exists)].reset_index(drop=True)
        video_filtered_len = len(self.split_df)
        if video_filtered_len != original_len:
            logger.warning(
                "Filtered %s HMDB rows with missing local videos for split %s.",
        ...
        if new_len != video_filtered_len:
            logger.warning(
                "Filtered %s HMDB rows with missing point features for split %s.",
                video_filtered_len - new_len,
                self.mode,
            )
        assert new_len > 0.95 * original_len, "Some videos or features are missing"
