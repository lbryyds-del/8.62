...
def make_frame_cluster_vis(video_frames, frame_id, feat_cluster_labels,
    ...
    overlay_cluster_mask = create_overlay_mask(
        original_frame,
    ...
    # Right subplot - semantic cluster with points
    ax2.imshow(overlay_cluster_mask)
    ax2.set_title('Semantic Cluster', fontsize=14)
    ax2.axis('off')
...
def get_points_from_clustering(args, video_frames, feat_extractor, debug_vis_root):
    ...
    semantic_feat_extractor = getattr(args, "semantic_feat_extractor", "dino")
    patch_features = feat_extractor(
        video_frames,
        model_type=semantic_feat_extractor,
    ).squeeze()
    ...
    if args.clustering_method == 'bipartite':
        feat_cluster_labels = get_temporal_bipartite_clusters(
                                        patch_features, merge_ratio=args.merge_ratio,
    ...
    for frame_id in range(n_frames):
        ...
        points, cluster_labels, component_labels= get_points_in_cluster(
                    args, feat_cluster_labels_frame.to(patch_features.device),
